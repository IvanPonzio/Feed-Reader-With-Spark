package com.example.spark.subscription;
...
public class Subscription {
	...
	public void setUrlsList(List<String> listUrlsRss){
		for (int i = 0; i < this.getSubscriptionsList().size(); i++) {
			// Chequeamos el tipo de url type de la suscripcion
			if(this.getSingleSubscription(i).getUrlType().equals("rss")) {
				// Recorremos los tipo de parametros de la url
				for(int j = 0; j < this.getSingleSubscription(i).getUrlParamsSize(); j++){
					// Agregamos estos url a la lista de urls
					listUrlsRss.add(this.getSingleSubscription(i).getFeedToRequest(j));
				}
			}
		}
	}
